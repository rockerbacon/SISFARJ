...
public class SecretarioTest {
	...
	public void lancarFiliacao () {
		try {
			...
			List<AssociacaoScript> lancados;
			...
			lancados = mapper.read(-1, AssociacaoScript.class, new Filter("asso_nome", "=", "test"));
			Assert.assertNotEquals(0, lancados.size());
			
			if (!lancados.isEmpty()) {
				Associacao entry = (Associacao)lancados.get(0).mapTo(new Associacao());
